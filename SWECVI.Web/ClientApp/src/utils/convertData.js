...
*/
export const objectsWithData = (data) => Object.keys(data)
...
export const getValueInObject = (data) => Object.values(data)
  .flatMap((subObject) => Object.values(subObject).map((subChildObject) => Object.values(subChildObject).map((item) => (item.value !== undefined
...
export const convertDataToInputV2 = (dataResp, dataObject) => {
  const result = dataResp.map((data) => {
    if (data.inputType === 0) {
      ...
      const {
        value, inputLabel, id, inputType, tabName,
      } = tmp;
  ...
  result.forEach((item) => {
    ...
    if (item.inputType === 7) {
      if ('Regionalitet' in item.value) {
        dataObject.valueOnlyRadioButton = {
          ...dataObject.valueOnlyRadioButton,
          Regionalitet: {
            ...dataObject.valueOnlyRadioButton.Regionalitet,
            ...item.value.Regionalitet,
          },
        };
      }
    }
